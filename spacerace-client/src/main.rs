...
///
/// **Controls:**
/// - Up Arrow or 'w': forward thrust (thrust = 1, rotation = 0)
/// - Down Arrow or 's': backward thrust (thrust = -1, rotation = 0)
/// - Left Arrow or 'a': rotate left (thrust = 0, rotation = 1)
/// - Right Arrow or 'd': rotate right (thrust = 0, rotation = -1)
        ...
        while event::poll(Duration::from_millis(0))? {
            if let Event::Key(key_event) = event::read()? {
                match key_event.code {
                    KeyCode::Up | KeyCode::Char('w') => thrust = 1,
                    KeyCode::Down | KeyCode::Char('s') => thrust = -1,
                    KeyCode::Left | KeyCode::Char('a') => rotation = 1,
                    KeyCode::Right | KeyCode::Char('d') => rotation = -1,
                    KeyCode::Char('q') => {
                        cleanup_terminal()?;
                        std::process::exit(0);
                    }
